...
本仓库独立于 `stock-view` / `stock-charts`。指数相关表历史上曾停更；红色火箭 job `sync_hongsehuojian_fill_validate` 可对指定标的**补缺** `index_daily_prices`，并刷新 `etf_valuation`（当日/5y/10y PE）。

## 当前范围

### ETF 日 K

- 从 `etf_pool` 读取当前池（排除黑名单后断言 20 只）。
