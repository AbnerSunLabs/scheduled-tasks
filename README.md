...
> 说明：ETF **价格**主源为 yfinance；补缺 / 估值 / 行业权重由红色火箭写入；汇率为 Frankfurter。不再使用 BaoStock / AKShare；`etf_daily` 不维护成交额；无交易日历表。

本仓库独立于 `stock-view` / `stock-charts`。指数日线表已删除；红色火箭 job `sync_hongsehuojian_fill_validate` 可刷新 `index_valuation`（当日/5y/10y PE）与行业权重。
...
- 数据源：华夏基金「红色火箭」站内 `fundex-quote` JSON（**非官方**，可能改版；直调 HTTP，不用页面爬虫）。
- 默认标的：`512170`（医疗ETF华宝）+ `399989.SZ`（中证医疗）。
- ETF：补缺写入 `etf_daily`（不复权 OHLC + 前/后复权，`price_source='hongsehuojian'`）。
- 行业权重：红色火箭主源刷新 `index_industry_weights`（该指数删旧写新；sw1/sw2/sw3）。
- **已有行不覆盖**，仅比对并写入 `sync_runs.meta` + `artifacts/sync_hongsehuojian_fill_validate_summary.json`。
- 入口：`python -m scheduled_tasks.jobs.sync_hongsehuojian_fill_validate`

### 官网双源校验

- 上交所（ETF）+ 中证（指数）vs 库；默认只比对告警，`--apply-official --yes` 才纠偏。
- 单标的默认：`512170` + `399989.SZ`；全池沪市：`--from-pool`（深市 `15xxxx` 暂无官网 client，会跳过）。
...
# 指数仪表盘共享行情表补 anon + authenticated 只读（兼容估值表改名前后）
...
5. **指数估值表更名**（`etf_valuation` → `index_valuation`；幂等、保留数据/RLS/视图依赖）：

```bash
psql "$DATABASE_URL" -f src/scheduled_tasks/models/migrations/20260719_rename_etf_valuation_to_index_valuation.sql
```

部署后只读验证：`index_valuation` 存在且有数据，`etf_valuation` 不存在；主键和公开读 policy 使用 `index_valuation_*` 名称。GitHub Actions 可单独勾选 `apply_index_valuation_rename_migration`。

6. **清理废弃列/表**（单独授权；勿与 rename 同批）：
...
只读验证：`etf_daily` **无** `amount` / `amount_source` / `amount_updated_at`；`trade_calendar` **不存在**。

> 历史 migration `20260715_etf_daily_amount_enrichment_and_trade_calendar.sql` 仅作旧库升级叙事保留；新库勿再依赖其中的日历/成交额列。

7. **表/列中文注释**（幂等；对象不存在则跳过；可随时重跑覆盖注释）：
...
8. 将 PostgreSQL 连接串写入 GitHub Secret `DATABASE_URL`。
9. 配置 `BARK_KEY`。
10. 手动跑一次 `同步 ETF 日 K 到 Supabase`，`mode=full` 补历史。
11. 手动跑一次 `同步汇率到 Supabase`，`mode=full` 补历史汇率。
...
python3 -m scheduled_tasks.jobs.sync_hongsehuojian_fill_validate --epsilon=0.001
# 默认 incremental=近 30 根（快）；全历史用 --mode=full；只刷估值：
...
# 可选覆盖默认标的：
python3 -m scheduled_tasks.jobs.sync_hongsehuojian_fill_validate --etf-code=512170 --index-code=399989.SZ
```
...
where schemaname = 'public'
  and tablename in (
    ...
    'grid_plans', 'review_entries', 'decision_logs', 'portfolio',
    'fx_rates'
  )
order by tablename;
```
