...
- **指数相关表 / 视图**：基线 DDL 见 `src/scheduled_tasks/models/schema.sql`。旧表 **`index_daily_prices` 已删除**（`migrations/20260718_drop_index_daily_prices.sql`）。红色火箭 job 可刷新 `index_valuation`（当日/5y/10y PE）与 `index_industry_weights`。
- **Live 相对本仓库 schema 的增量**（以 Studio / `information_schema` 为准；**本仓库 `schema.sql` / `migrations/` 尚未完全收录**）：
  ...
  - 新表 `index_daily_metrics`：指数日指标（`close` / `pe_ttm` / `pb` + 来源列）；FK → `indices(code)`；`anon`/`authenticated` SELECT。
...
- Live 另有：`index_daily_metrics_select_anon` / `index_daily_metrics_select_authenticated`；`indices` 上仍保留历史 policy `Allow public read indices`（与 `indices_select_*` 并存，效果均为公开 SELECT）。
- `schema.sql` 本身不含完整 RLS；已有库请执行 `20260710_cockpit_ledger_and_fx_rates.sql`，并补跑 `20260718_etf_pool_authenticated_read.sql` 与 `20260718_index_market_anon_read.sql`。
...
| 类型 | 名称                     | 说明                                                 | 本方案状态                          |
| ---- | ------------------------ | ---------------------------------------------------- | ----------------------------------- |
| 表   | `indices`                | 指数元数据（含估值可用性状态列）                     | 红色火箭可 ensure 单行              |
| 表   | `index_daily_metrics`    | 指数日指标（收盘点位 / PE TTM / PB）                 | **Live 已有**；本仓库 schema 未收录 |
...
| `fx_rates`               | **33**     | **2026-07-03 → 2026-07-17**；每日 3 对（USD/CNY、USD/HKD、HKD/CNY） |
| `sync_runs`              | **183**    | 含历史 `sync_indices` / `sync_etf_kline_baostock` 等旧 job 名       |
...
| `sync_fx_rates_frankfurter`        | 5        | 0                    | 2026-07-17      |
...
旧全市场 TuShare `sync_indices` 链路已删除。**`index_daily_prices` 已删除**（`20260718_drop_index_daily_prices.sql`）。Live 另建 **`index_daily_metrics`**（见下；与已删的日线表职责不同：合入 close + PE/PB，且目前仅沪深 300 有数据）。红色火箭 job `sync_hongsehuojian_fill_validate`（见 [hongsehuojian-fill-validate.md](./hongsehuojian-fill-validate.md)）可 ensure `indices`、upsert `index_valuation`，并以红色火箭主源刷新 `index_industry_weights`。`index_daily_valuations` 已删除；指数视图估值仍挂 `index_valuation`（视图收盘相关列仍恒为 null，**未**改挂 `index_daily_metrics`）。
...
### `index_daily_metrics` — 指数日指标（Live）

> **Live 已存在**；本仓库 `schema.sql` / `migrations/` **尚未收录** DDL。表注释：指数日指标（收盘点位、PE TTM、PB）。与已删除的 `index_daily_prices` 不同：同一主键下可同时存点位与估值字段。
...
**RLS：** `index_daily_metrics_select_anon` / `index_daily_metrics_select_authenticated`（`USING (true)`）。
...
| `price_source`                                                                    | `text`              | 可空                      | 不复权 OHLC/volume 来源：`yfinance`（主写）/ `hongsehuojian`（红色火箭补缺 INSERT）/ `adj_gap_fill`（历史缺复权回填）；`adj_check` 不更新 |
...
index_daily_metrics（Live；本仓库尚无对应 job / schema）
    └─ 当前数据来源标注为 hongsehuojian；仅 000300.SH 有行
```
