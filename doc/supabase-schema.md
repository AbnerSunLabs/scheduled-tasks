...
| `price_source`                                                                    | `text`              | 可空                      | 仅表示不复权 OHLCV 来源；本 job 写 `'baostock'`；`adj_check` 不更新 |
| `updated_at`                                                                      | `timestamptz`       | NOT NULL, DEFAULT `now()` | 更新时间                                                            |
...
etf_pool_snapshots（只读，全表）
    │
    ▼
BaoStock（adjustflag 3/2/1）
    │
    ├─ full / incremental → etf_daily（三种价 + price_source）
    ├─ adj_check          → etf_daily（仅 UPDATE *_qfq/*_hfq）
    └─ sync_runs + artifacts/sync_etf_kline_summary.json → Bark
```

同步入口：`python -m scheduled_tasks.jobs.sync_etf_kline_baostock`（workflow：`同步 ETF 日 K 到 Supabase`）。
