...
（live rename 迁移未执行，`etf_pool` 尚不存在；本 Spike 只借该表拿真实池内 25 只标的与分类，
不涉及任何写操作，与生产表名切换无关）。
...
| 代码 | 名称 | 分类 | 交易所 |
| ---- | ---- | ---- | ------ |
| 510300 | 沪深300ETF | 宽基 | SSE |
| 159915 | 创业板ETF | 宽基 | SZSE |
| 512480 | 半导体ETF | 行业 | SSE |
| 159928 | 消费ETF | 行业 | SZSE |
| 513100 | 纳指ETF | 跨境 | SSE |
| 159920 | 恒生ETF | 跨境 | SZSE |

### 2.2 池外能力探针（**不在正式池，仅验证源能力**）

| 代码 | 名称 | 说明 |
| ---- | ---- | ---- |
| 511010 | 国债ETF | 债券类探针，**未进入正式池** |
| 518880 | 黄金ETF | 商品类探针，**未进入正式池** |

### 2.3 覆盖结果（AKShare，窗口 ≤366 天，本次拉取范围约近 420 天）

| 代码 | 实际交易日数 | amount 非空率 | ≥250 日 | 非空率≥90% |
| ---- | ------ | ------ | ------ | ------ |
| 510300 | 971（含 5 年深度样本） | 100% | ✅ | ✅ |
| 159915 | 989（含 5 年深度样本） | 100% | ✅ | ✅ |
| 512480 | 281 | 100% | ✅ | ✅ |
| 159928 | 243 | 100% | ❌（差 7 日，因一个窗口重试后仍失败） | ✅ |
| 513100 | 281 | 100% | ✅ | ✅ |
| 159920 | 38 | 100% | ❌（主窗口重试后仍失败，仅剩尾部小窗口） | ✅ |
| 511010（池外） | 281 | 100% | ✅（单列，不计入正式池门禁） | ✅ |
...
- `510300`：5 年窗口分段拉取，其中 4 个自然年窗口成功（2021-06~2025-06，合计 971 行），最近一年
  的窗口因网络失败被跳过（见 §3）；已获数据 amount 非空率 100%。
- `159915`：5 年窗口 6 段中 5 段成功（仅 2021-06~2022-06 失败），合计 989 行，amount 非空率 100%。
- **深度门禁 PASS**：两只标的中至少一只（实际两只都）在已成功拉取的数据上 amount 非空率 ≥90%。
...
- 平均单次请求延迟 288.5ms，P95 503.2ms（不含 sleep/backoff 等待时间）
- 52 次总尝试中，**28 个窗口里 9 个窗口需要重试**（即首次请求失败但重试后成功）
- **6 个窗口在 4 次尝试（首次 + 3 次重试，2s/4s/8s ± 20% jitter）后仍失败**，
  **窗口级失败率 21.4%（门禁要求 ≤10%，FAIL）**
...
- 复测验证：对失败标的（如 `159920`）单独追加 8 次请求（间隔 2s），前 3 次失败、后 5 次全部成功，
  证实是概率性抖动而非该标的/该请求参数本身不可用

### 3.2 BaoStock（单列统计，不稀释主源分母）

- `query_history_k_data_plus`：登录成功，全部 16 个窗口请求 **0 次失败**（100% 成功率），
  平均延迟 153.6ms、P95 384.8ms——TCP 协议本身在本机网络下反而比 AKShare 的 HTTPS 更稳定
...
- 可映射为 `trade_calendar.trade_date = calendar_date`，`is_open = is_trading_day`，`market='CN'`（固定）
...
7/8 只标的在 BaoStock 覆盖窗口（2026-01 起）与 AKShare 数据有重叠日期，抽样最近 5 个交易日对比：

| 代码 | 抽样天数 | 相对差异 |
| ---- | ---- | ---- |
| 159915 | 5 | 全部 0.0（完全一致） |
| 512480 | 5 | 全部 0.0（完全一致） |
| 513100 | 5 | 全部 0.0（完全一致） |
| 159928 / 159920 / 511010 / 518880 | 5 | 全部 0.0（完全一致，详见 raw JSON） |
| 510300 | — | 本次运行因 AKShare 最近一年窗口失败（§3.1），与 BaoStock 覆盖区间未产生重叠，暂无法对比 |

**AKShare 与 BaoStock 在有重叠数据的交易日上 amount 完全一致（relative_diff=0.0）**——两者最终都
取自交易所原始行情，是强正向信号：只要能拿到数据，两源口径一致，可互相验证。

## 5. 硬门禁判定详情（§4.4，对应脚本 `gate_evaluation`）

```
unit:       PASS  (min pass_rate = 1.0, 要求 >= 0.95)
coverage:   FAIL  (159928: 243日<250；159920: 38日<250；其余6只全部达标)
depth:      PASS  (510300: 971日100%非空；159915: 989日100%非空)
stability:  FAIL  (AKShare 窗口失败率 21.4% > 10%；BaoStock 单列 0% 失败，未计入分母)
...
2. **国内 Hermes 机器复现**：`git pull` 本分支后执行：
   ...
   ```
   预期国内网络下 AKShare 到东财的连接失败率会显著低于本次的 21.4%（本次失败模式是连接级
   RST，与地域网络质量强相关）；若失败率降到 ≤10% 且覆盖门禁两只标的补齐 ≥250 日，则可判
