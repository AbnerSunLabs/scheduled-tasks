...
`valuation-only` upsert `index_valuation` + `index_daily_metrics` PE/PB（`timeInterval=last_10_years`）；**不**拉长 K、**不**刷新 `index_industry_weights`。历史空洞用 `--mode=full`（`max`）回填。

红色火箭估值窗口：`max`（全量，偏周频）、`last_10_years`（近 10 年日频）、`last_5_years`。
